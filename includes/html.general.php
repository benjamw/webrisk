...
function get_header($meta = null) {
	...
	if ($show_menu) {
		if ( ! $menu_data) {
			...
			$menu_data['archived'] = Game::get_archived_count( );
	...
	if ($show_menu) {
		...
		<div id="menu">
			<ul>
				...
				<li'.get_active('archive').'><a href="archive.php'.$GLOBALS['_?_DEBUG_QUERY'].'" title="(Game Count)">Game Archives ('.$menu_data['archived'].')</a></li>
